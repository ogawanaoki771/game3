...
class Fighter:
    ...
    def control_user(self, keys, fighters):
        ...
        # 既存AIと同じ脳をユーザーの身体にも一時的に使う。
        target = self.target(fighters)
        if target is None:
            return 0.0, 0.0, False, False

        if self.brain is None:
            # YOU自身は通常AI brainを持たないので、必要時だけ生成。
            self.brain = AdvancedAIMemory(self.pid, "tactical")

        action = self.brain.choose_advanced(
            self,
            target,
            [f for f in fighters if f.alive and f.pid != self.pid],
            self.world
        )
...
def draw_hud():
    ...
    hud.write(
        ...
        "R reset   E forget   P pause   |   input = AI assist / no input = AI auto",
        font=("Arial", 9, "normal"),
    )
