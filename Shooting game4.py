...
class ActionLawMemory:
    """Learns action -> observed delta-state relations.

    The memory does not ask whether an action was good or bad.
    It records what actually changed after the action:

        (state, action) -> delta-state

    Delta-state is approximately:
        dx, dy, dvx, dvy, dhp_me, dhp_target, d_distance

    This lets the controller choose actions by predicting the physical
    consequence they tend to create instead of using tactical distance rules.
    """
    ...
    def make_snapshot(me, target):
        ...
        return {
            ...
            "vx": me.vx, "vy": me.vy,
            "hp": me.hp,
            "td": distance(me.x, me.y, target.x, target.y),
        }
    ...
    def mean_delta(values):
        if not values:
            return None
        n = float(len(values))
        return tuple(sum(v[i] for v in values) / n for i in range(len(values[0])))

    @staticmethod
    def delta_error(a, b):
        if a is None or b is None:
            return 1.0
        weights = (1.0, 1.0, 0.70, 0.70, 0.08, 0.0, 0.55)
        err = 0.0
        for i, w in enumerate(weights):
            err += abs(a[i] - b[i]) * w
        return err

    def observe(self, state, action, before, after):
        if state is None or action is None or before is None or after is None:
            return

        delta = self.make_delta(before, after)
        key = (state, action)
    ...
    def predict(self, state, action):
        bucket = self.samples.get((state, action))
        if not bucket:
            return None, 0.0, 0
        delta = self.mean_delta(bucket)

        # Confidence grows with repeated observations, but never becomes 1.
        confidence = 1.0 - math.exp(-len(bucket) / 6.0)
        return delta, confidence, len(bucket)

    def action_confidence(self, state, action):
        _, confidence, count = self.predict(state, action)
        return confidence if count else 0.0

    def known_actions(self, state):
        result = []
        for action in ACTIONS:
            predicted, confidence, count = self.predict(state, action)
            if predicted is not None:
                result.append((action, predicted, confidence, count))
        return result

    def best_action_for_delta(self, state, desired_delta):
        best = None
        for action in ACTIONS:
            predicted, confidence, count = self.predict(state, action)
            if predicted is None:
                continue
            err = self.delta_error(predicted, desired_delta)
            score = confidence / (1.0 + err)
            if best is None or score > best[0]:
                best = (score, action, predicted, confidence)
        return best
...
# ------------------------------------------------------------
# DELTA-STATE TRANSFORMATION CONTROLLER
# ------------------------------------------------------------
class TransformationController:
    """Selects actions from predicted consequences, not tactical values.

    The controller builds a temporary desired transformation from the current
    world geometry. It never says "distance < 155 => retreat".

    It instead asks: which learned action is capable of producing the desired
    movement of the world state? Unknown actions are used as experiments.
    """

    def __init__(self):
        self.experiment_bias = 0.50
        self.control_bias = 0.80
        self.novelty_bias = 0.35

    def desired_delta(self, me, target, world):
        """Construct a weak physical objective, not an action rule.

        The target relation only specifies a desired direction of state change.
        No action name is selected here.
        """
        if target is None:
        ...
        # A smooth desired radial state instead of hard thresholds.
        # Around the target, tangential motion becomes more useful; far away,
        # radial correction dominates. The controller predicts the consequence
        # and chooses an available action that approximates it.
        radial = clamp((d - 260.0) / 260.0, -1.0, 1.0)
        tangent = math.sin(world.time * 0.045 + me.pid * 1.37) * 0.35

        return (
            nx * radial * 4.0 - ny * tangent * 2.2,
            ny * radial * 4.0 + nx * tangent * 2.2,
            0.0,
            0.0,
            0.0,
            0.0,
    ...
    def score(self, state, action, memory, desired_delta, curiosity):
        predicted, confidence, count = memory.predict(state, action)

        if predicted is None:
            return self.experiment_bias * (0.30 + curiosity)

        error = memory.delta_error(predicted, desired_delta)
        fit = 1.0 / (1.0 + error)
        controllability = confidence * fit
        novelty = (1.0 - confidence) * curiosity

        return (
            self.control_bias * controllability
...
class TransformationAI:
    ...
    def __init__(self, pid, style):
        ...
        self.action_laws = ActionLawMemory()
    ...
    def observe_all(self, me, fighters, world):
        ...
        # Learn the physical consequence of the previous action by comparing
        # the snapshot taken before that action with the current world state.
        current_snapshot = self.action_laws.make_snapshot(me, target)
        if (
            self.action_laws.last_state is not None
            and self.action_laws.last_action is not None
            and self.action_laws.last_snapshot is not None
        ):
        ...
        state = self.memory.encode_state(me, target, world)
        event = self.current_event(me, target, world)
        self.memory.observe(me, target, world, event)

        self.action_laws.last_state = state
        self.action_laws.last_action = self.last_action
        self.action_laws.last_snapshot = current_snapshot
    ...
    def score_action(self, action, me, target, enemies, world):
        """Score an action by predicted delta-state fit.

        There is no distance->action table here. The current geometry only
        defines a desired change. Learned action laws supply the candidate
        physical consequences.
        """
        state = self.memory.last_state
        desired = self.controller.desired_delta(me, target, world)
    ...
    def choose(self, me, target, fighters, world):
        ...
        # At the beginning, many action/state pairs are unknown. Exploration
        # discovers the action-conditioned laws. Later, controllability takes
        # over automatically as confidence accumulates.
        uncertainty = self.memory.curiosity
        if random.random() < 0.10 + 0.22 * uncertainty:
            top_k = 3 if uncertainty > 0.45 else 2
...
class Fighter:
    ...
    def control_ai(self, fighters):
        ...
        # The controller predicts state change; aiming is the minimum physical
        # affordance needed to express the chosen transformation.
        self.angle = self.brain.intercept_angle(self, target)

        state = self.brain.memory.last_state
        predicted, confidence, count = self.brain.action_laws.predict(state, action)
        ...
        if predicted is not None:
            dx, dy, dvx, dvy, _dhp, _dhtarget, _dd = predicted
            # Reuse observed physical displacement directly.
            mx += dx / max(1.0, MOVE)
            my += dy / max(1.0, MOVE)

            # Acceleration change is also a learned consequence, so preserve a
            # small amount of it when the displacement itself is weak.
            mx += dvx * 0.35
            my += dvy * 0.35

        # No hard-coded distance rule. Unknown actions are experiments.
        if predicted is None:
            desired = self.brain.controller.desired_delta(
                self, target, self.world
            )
            mx = desired[0] / max(1.0, MOVE)
            my = desired[1] / max(1.0, MOVE)

        # Action-specific actuation is deliberately minimal. The controller
        # learned whether the primitive tends to create the required change.
        if action == "SHOOT":
            shoot = self.cooldown <= 0
        elif action == "DASH":
            dash = self.dash_cd <= 0

        # Keep motion bounded while preserving the learned direction.
        length = math.hypot(mx, my)
        if length > 1.0:
            mx /= length
            my /= length

        _ = (enemies if False else None, confidence, count)
    ...
    def control_user(self, keys, fighters):
        ...
        self.last_action = "AUTO:" + self.last_action
        self.assist_active = True
...
def draw_hud():
    ...
    hud.color("#91a4c2")
    hud.write(
        f"OBSERVE / PREDICT / COMPARE / LEARN CHANGE / ACT    "
